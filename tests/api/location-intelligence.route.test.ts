...
describe("GET /api/location-intelligence", () => {
  it("returns the application error envelope for invalid latitude", async () => {
    const response = await GET(new Request("http://localhost/api/location-intelligence?lat=91&lng=102&radius=25"));
  ...
  it("returns the application error envelope for invalid radius", async () => {
    const response = await GET(new Request("http://localhost/api/location-intelligence?lat=4&lng=102&radius=12"));
    expect(response.status).toBe(400);
