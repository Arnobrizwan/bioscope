...
import { AlertTriangle, Bot, CloudRain, Droplets, Leaf, LoaderCircle, MapPinPlus, Save, Sun, Thermometer, Trees } from "lucide-react";
...
export function IntelligencePanel({ data, loading }: { data: LocationIntelligence | null; loading: boolean }) {
  ...
  async function saveLocation() {
    ...
    try {
      ...
      if (!response.ok) throw new Error(result.error?.message || "Unable to save location.");
      setSaveStatus("saved");
      setSaveMessage("Location saved to your dashboard.");
    } catch (error) {
      setSaveStatus("idle");
      setSaveMessage(error instanceof Error ? error.message : "Unable to save location.");
    }
  }
