...
export async function GET(request: Request) {
  try {
    const limit = checkRateLimit(`location:${requestIdentity(request)}`, 30, 60_000);
    if (!limit.allowed) throw new AppError("RATE_LIMITED", "Too many analysis requests.", 429);
    ...
    return Response.json({ data: intelligence }, { headers: { "Cache-Control": "private, max-age=60" } });
  } catch (error) {
