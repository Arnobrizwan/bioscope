...
const groupSchema = z.enum(["birds", "mammals", "reptiles", "amphibians", "plants", "insects", "other"]);
...
export const fieldBriefInputSchema = z.object({
  ...
  taxonomicDistribution: z.object({
    birds: z.number().nonnegative(), mammals: z.number().nonnegative(), reptiles: z.number().nonnegative(),
    amphibians: z.number().nonnegative(), plants: z.number().nonnegative(), insects: z.number().nonnegative(), other: z.number().nonnegative(),
  }),
  environmentalSnapshot: z.object({
    temperature: z.number().optional(), precipitation: z.number().optional(), humidity: z.number().optional(), solarRadiation: z.number().optional(),
    period: z.object({ start: z.string(), end: z.string() }),
    units: z.object({ temperature: z.literal("°C"), precipitation: z.literal("mm/day"), humidity: z.literal("%"), solarRadiation: z.literal("kWh/m²/day") }),
  }).nullable(),
});
